...
public class ArmWithEncoder {
	...
	public ArmWithEncoder () {
		...
		encoder = new Encoder(RobotMap.ARM_ENCODER_PORT_ONE, RobotMap.ARM_ENCODER_PORT_TWO);
		positionController = new PIDController(P, I, D, 0, encoder, speedController);
	...
	private class DetermineIfCalibrationComplete extends TimerTask {
		...
		public void run() {
			...
			System.out.println("Arm position last=" + lastPositionRead + " current=" + positionRead);
			final double angleTolerance = 1;
			if ( Math.abs(lastPositionRead - positionRead) < angleTolerance ) {
				// we have reached the up position, so set the encoder position to 0 degrees, deenergize the motor, 
